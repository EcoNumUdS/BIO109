...
Certaines opérations peuvent être tailel dépendantes
...
Certaines fonctions sur R (voir Visser et al. 2015) sont optimisées. L'exemple suivant montre la puissance de la fonction 'colSums'. 

```{r}
f1 <- function(x) {
	res <- numeric(ncol(x))
	for(j in 1:ncol(x)) {
		for(i in 1:nrow(x)) res[x] = res[j] + x[i,j]
	}
}
