...
PROGRAM DEVOIR
  ...
  FOR i IN 1:20     <--------- Correspond au nombre de tirages
    r = RANDOM()    <--------- Tirage d'une valeur aléatoire 
    WHILE r < 1     <--------- Si le chiffre est plus petit que 1 
      IF r > 100    <--------- Et qu'il est plus grand que 100
        r = RANDOM() 
      END IF
    END WHILE

    nombres[i] = r  <--------- Enregistre la valeur
...
PROGRAM DEVOIR (SUITE)
  ...
  FOR position IN 1:20 
    somme = somme + nombres[position]   <--------- Somme cumulative
  END FOR

  longueur = LENGTH(nombres)            <--------- Longueur du vecteur
  moyenne = somme / longueur            <--------- Calcul de la moyenne
