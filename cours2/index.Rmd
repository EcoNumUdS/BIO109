...
PROGRAM DEVOIR
  ...
  FOR loop IN 1:1000 <--------- Définir un grand nombre d'itérations
    r = RANDOM()
    IF r >= 1 
      IF r <= 100
        nombres[i] = r
    ...
    IF LENGTH(nombre) == 20 <-- Vérifier si le vecteur est complet
      BREAK
    END IF
  END FOR
```

Ça fonctionne, mais c'est pas très élégant... Voyons comment faire mieux.
...
PROGRAM DEVOIR
  ...
  WHILE LENGTH(nombres) < 20           <--- Utiliser WHILE
    r = RANDOM()
    IF r >= 1 AND r <= 100             <--- Utiliser AND pour coupler les conditions
      nombres[i] = r
      i = i + 1
    END IF
  END WHILE
...
## On peut utiliser une condition pour sélectionner des valeurs:
...
# Note sur les opérateurs logiques
...
**NOTE** : Pour plus de détails sur les opérateurs logiques, voir [operators](https://www.statmethods.net/management/operators.html)

---

# Note sur les opérateurs logiques

## On peut combiner plusieurs conditions avec les opérateurs `&` et `|`

```{r}
vect <- c(2,4,6,8,10)
# 'condition 1 et condition2'
vect[vect > 5 & vect < 10]
...
Mais chaque colonne doit être du même type
